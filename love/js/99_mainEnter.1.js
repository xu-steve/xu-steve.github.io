...
var car2 = {
    ...
    _iPhoen: RegExp("iPhone").test(navigator.userAgent) || RegExp("iPod").test(navigator.userAgent) || RegExp(
        "iPad").test(navigator.userAgent) ? true : false,
    ...
    page_position: function (e, moveP, $self) {
        ...
        function init_next(node) {
            ...
            if (car2._movePosition == 'up') {
                ...
                node[0].style[car2._prefixStyle('transform')] = 'translate(0,-' + Math.max($(window).height(),
                    $(node[0]).height()) + 'px)' + _translateZ;
    ...
    page_translate: function (node) {
        ...
        y_2 = y_2 / 5;
        node[1].style[car2._prefixStyle('transform')] = 'translate(0,' + y_2 + 'px)' + _translateZ + ' scale(' +
            scale + ')';
    },
    ...
    page_fial: function () {
        ...
        if (car2._movePosition == 'up') {
            car2._page.eq(car2._pageNext)[0].style[car2._prefixStyle('transform')] = 'translate(0,' + $(window)
                .height() + 'px)' + _translateZ;
        } else {
            car2._page.eq(car2._pageNext)[0].style[car2._prefixStyle('transform')] = 'translate(0,-' + $(window)
                .height() + 'px)' + _translateZ;
        }

        car2._page.eq(car2._pageNow)[0].style[car2._prefixStyle('transform')] = 'translate(0,0)' + _translateZ +
            ' scale(1)';
        ...
        setTimeout(function () {
            ...
            if (car2._map.find('div').length >= 1) {
                ...
                setTimeout(function () {
                    ...
                    if (!car2._mapValue) car2.addMap(detal, latitude, longitude, fnOpen,
                        fnClose);
                }, 500)
            } else return;
        }, 100)
    ...
    Txt_init: function (node) {
        ...
        node.find('.j-txt').each(function () {
            ...
            if ($(this.parentNode).hasClass('z-left')) p[0].style[car2._prefixStyle('transform')] =
                'translate(0,' + height_p + 'px)';
    ...
    bigTxt_extand: function () {
        $('body').on('click', '.j-title', function () {
            ...
            detail.hasClass('z-show') ? ($(this).find('.txt-arrow').addClass('z-toggle')) : ($(this).find(
                '.txt-arrow').removeClass('z-toggle'))
        })
    }(),
    ...
    Txt_back: function () {
        $('body').on('click', '.m-page', function (e) {
            ...
            if (page.find('.j-txt').length <= 0 || node.parents('.j-txt').length >= 1 || node.hasClass(
                    'bigTxt-btn') || node.parents('.bigTxt-btn').length >= 1) return;
            ...
            txtWrap.each(function () {
                ...
                detail.hasClass('z-show') ? (arrow.addClass('z-toggle')) : (arrow.removeClass(
                    'z-toggle'));
            })
        })
    }(),
    ...
    input_form: function () {
        $('body').on('click', '.book-bd .bd-form .btn', function () {
            ...
            $('.book-bg').on('click', function (e) {
                ...
                if (node.parents('.book-form').length >= 1 && !node.hasClass('j-close-img') &&
                    node.parents('.j-close').length <= 0) return;
    ...
    plugin: function () {
        ...
        // 音符飘逸
        $('#coffee_flow').coffee({
            steams: ["<img src='../images/audio_widget_01@2x.png' />",
                "<img src='../images/audio_widget_01@2x.png' />"],
            steamHeight: 100,
            steamWidth: 44
        });
